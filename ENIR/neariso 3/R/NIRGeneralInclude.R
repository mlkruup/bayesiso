...
neariso2 = function(y, x, maxBreaks=100, lambda=NULL)
{
  if(!is.vector(y)) {
    stop("y has to be a vector")
  }
  ...
  solObj = .Call("NIR2",as.numeric(y), as.numeric(x),PACKAGE="neariso")
  if(is.null(lambda))
  {
    lambda <- nearisoGetBreakpoints(solObj, maxBreaks)
  }
...
nearisoGetBreakpoints <- function(nearisoPathObj, maxBreaks=100) {
    ...
    if(lambdaList[1] == -1) {
        lambdaList[1] <- 0
        if (length(lambdaList)>1 && lambdaList[1] == lambdaList[2]){
          lambdaList <- lambdaList[2:length(lambdaList)]
        }
    }
    else {
